...
class Lexer:
    ...
    def get_next_token(self):
        ...
        while self.current_char is not None:
            ...
            if self.current_char == "*":
                self.advance()
                return Token(TokenType.MUL, "*")  # Розпізнаємо '*'

            if self.current_char == "/":
                self.advance()
                return Token(TokenType.DIV, "/")  # Розпізнаємо '/'

            if self.current_char == "(":
                self.advance()
                return Token(TokenType.LPAREN, "(")  # Розпізнаємо '('

            if self.current_char == ")":
                self.advance()
                return Token(TokenType.RPAREN, ")")  # Розпізнаємо ')'
...
class Parser:
    ...
    def factor(self):
        """Парсер для 'factor' правил граматики: ціле число або вираз у дужках."""
        ...
        if token.type == TokenType.INTEGER:
            ...
            node = self.expr()  # Обробляємо підвираз у дужках
...
class Interpreter:
    ...
    def visit_BinOp(self, node):
        """Обчислюємо значення для BinOp вузлів."""
    ...
    def visit_Num(self, node):
        """Повертаємо числове значення."""
        return node.value

    def interpret(self):
        """Запускаємо процес інтерпретації."""
